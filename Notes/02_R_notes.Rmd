...
# note colMeans is actually another apply function: colMeans = apply(x, 2, mean)
```
...
Comparing results from `sapply` + *anonymous* function vs. tidyverst piping
* both arrive at similar results
    + sapply more condensed and faster to type but less intuitive
    + tidyverse easier to read and understand but longer to compose
```{r}
...
Interactive debugging tools (s)

`traceback()` # Prints out fn call stack, must call immediately after the error
`recover()`   # error handler: stops execution at error and opens fn call stack

`debug()`     # allows you step through a fn 1 line at a time, "n" advances to next line
`browser()`   # suspends execution and puts fn in "debug" mode
`trace()`     # insert debugging code into fn w/o editing fn itself

`print()`     # have to go back and delete when done debugging
`cat()`
